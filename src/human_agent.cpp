...
HumanAgent::HumanAgent(const Map &map, Console &console, int id, std::string name, sf::Color color) : IAgent(map, id, name, color),
                                                                                                      console(console)
{
}
...
const Territory *HumanAgent::chooseTerritory(const std::vector<Territory *> &valid_territories, bool choose_unoccupied)
{
  ...
  {
    std::string territory_name;
    territory_name = console.prompt("Choose a territory to reinforce:");
    const Territory *territory = map.getTerritory(territory_name);
    while (territory == nullptr || territory->getOccupierId() != id)
    {
      std::string msg = (territory == nullptr)
                            ? territory_name + " is not a territory. Choose a territory to reinforce.:"
                            : "You do not own " + territory_name + ". Choose a territory to reinforce.:";
      territory_name = console.prompt(msg);
      territory = map.getTerritory(territory_name);
    }
  }
...
{
  std::string territory_name;
  std::string prompt_msg = "You have " + std::to_string(total_reinforcements) + " reinforcements available." + " What territory would you like to reinforce?";
...
std::tuple<const Territory *, const Territory *, int> HumanAgent::attack()
{
  ...
  while (attacking_units < 1 || attacking_units > 3 || from->getUnits() <= attacking_units)
  {
    ...
    {
      err_msg = "You need " + std::to_string(attacking_units + 1) + " units in " + from_name + " but there are only " + std::to_string(from->getUnits()) + "!";
    }
...
{
  std::string prompt_msg = defender->getName() + " is being attacked from " + attacker->getName() + " by " + std::to_string(attacking_units) + " units! How many units will defend?";
