...
RandomAgent::RandomAgent(const Map &map, int id, std::string name, sf::Color color) : IAgent(map, id, name, color)
{
...
const Territory *RandomAgent::chooseTerritory(const std::vector<Territory *> &valid_territories, bool choose_unoccupied)
{
  int rand_index = getRandomInt(0, valid_territories.size() - 1);
  return valid_territories.at(rand_index);
}
